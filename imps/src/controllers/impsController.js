const impsModel = require("../models/imps");
...
const createimps = async (req, res) =>{
...
const {bankname,receiverano,ifsc,transamount} = req.body;
    ...
    console.log(mydata1) 
    const mydata2 = await fetch("http://localhost:5001/users/changebyimps/"+j,  {
        method: 'PUT',
        headers: {
...
const updateimps = async (req, res) =>{
    ...
    try {
        await impsModel.findByIdAndUpdate(id, newNote, {new : true});
...
const deleteimps = async (req, res) =>{

const id = req.params.id;
try {
    
    const note = await impsModel.findByIdAndRemove(id);
...
try {
    ...
    const notes = await impsModel.find({senderano : req.userId});
...
try {
    ...
    const notes = await impsModel.find({});
    res.status(200).json(notes);
